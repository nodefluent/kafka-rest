...
class KafkaRest extends EventEmitter {
  ...
  _buildWebServer(config = null) {
    ...
    this.webApp.post("/consumers/:consumer/instances/:instance/positions", (req, res) => {
      // body: {"offsets": [{"topic": "test","partition": 0,"offset": 1}]}
      res.status(204).end();
    });
    ...
    this.webApp.post("/consumers/:consumer/instances/:instance/positions/end", (req, res) => {
      // body: {"topics":["test"]}
      res.status(204).json({
        "partitions": [
          {
