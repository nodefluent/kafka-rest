...
const getRouter = (app) => {
  ...
  router.post("/:consumer", (req, res) => {
    ...
    app.kafka.getConsumer(req.params.consumer, [], earliestElseLatest, body.maxWindowSize ? body.maxWindowSize : 1000, body.format).then(() => {
  ...
  router.get("/:consumer/instances/:instance/records", async (req, res) => {
    ...
    if(consumer.messageFormat === "avro"){
      return res.status(404).end();
    }

    const tries = 3;
    const messages = await consumer.getMessages(req.query.timeout/tries, tries);
...
const subscribeAndResponce = (app, req, res, topics)=>{
  app.kafka.getConsumer(req.params.consumer).then(consumer => {
