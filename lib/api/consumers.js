...
const getRouter = (app) => {
  ...
  router.post("/:consumer/instances/:instance/subscription", (req, res) => {
    ...
    subscribeAndRespond(app, req, res, req.body.topics);
  });
  ...
  router.get("/:consumer/instances/:instance/records", async (req, res) => {
    ...
    const tries = 3;
    let timeout = req.query.timeout ? parseInt(req.query.timeout, 10) : 500;
    timeout = !isNaN(timeout) ? timeout/tries : 500;
    const max_bytes = req.query.max_bytes;
    debug("Get messages", {timeout, tries, max_bytes});
    const messages = await consumer.getMessages(timeout, tries, max_bytes);
  ...
  router.post("/:consumer/instances/:instance/positions/beginning", (req, res) => {
    ...
    subscribeAndRespond(app, req, res, topics);
  });
  ...
  router.post("/:consumer/instances/:instance/positions/end", (req, res) => {
    ...
    const topics = getTopicsFromPartitions(req.body.partitions);
    subscribeAndRespond(app, req, res, topics, false);
  });
...
const subscribeAndRespond = (app, req, res, topics, earliestElseLatest = true)=>{
